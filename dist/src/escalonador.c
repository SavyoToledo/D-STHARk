...
void PushTask(Ttask t){
    ...
    if(t.taskType != TERMINATOR_ID){
		...
		if(workerCount[1] != 0){
			...
			if (workerCount[0] != 0)
				workers_position_vector++;
			
			gpuTime_tmp += transferSpeed[workers_position_vector];
		...
		if(workerCount[2] != 0){
			...
			micTime_tmp += transferSpeed[workers_position_vector];
